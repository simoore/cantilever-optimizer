...
class NewCompactTopology(object):
    ...
    def update_topology(self, xs):
        ...
        if self._is_connected is True:
            ...
            self._xtip = 1e6 * (2 * self.a * nelx)
            self._ytip = 1e6 * (2 * self.b * (ytip_norm - 0.05))
    ...
    def _apply_regularization(self, topology, symmetry=True):
        """
        ...
        if symmetry is True:
            new_topology = np.vstack((topology, np.flipud(topology)))
